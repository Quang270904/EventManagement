    ...
    <script>
        ...
        function updateEventTable(events, page = 1) {
            ...
            if (events.length > 0) {
                ...
                <td>${event.status}</td>
                <td>
                     <a href="/event_manager/event/${event.id}/detail" class="btn btn-info btn-sm viewUserBtn">View</a>
                    <a href="/event_manager/event/${event.id}/edit" class="btn btn-warning btn-sm">Edit</a>
        ...
        $(document).on('click', '.deleteEvent', function(event) {
            ...
            if (confirm("Are you sure you want to delete this event?")) {
                $.ajax({
                    type: "POST",
                    url: "/event_manager/event/" + eventId + "/delete",
                    data: {
